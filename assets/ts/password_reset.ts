(() => {
    const main_input = document.querySelector<HTMLInputElement>("main input")!;
    const main_button = document.querySelector<HTMLButtonElement>("main button.login-link")!;
    ...
    const pErrorTimeout = document.querySelector<HTMLElement>(".error-timeout")!;
    ...
    function showSnackbar(type: 'success' | 'failure' | 'error') {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const data: SendResetPasswordResponse = await response.json();
