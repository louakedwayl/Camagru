...
(() => {
    ...
    function resizeSticker(sticker, scale) {
        ...
        if (newW < 30 || newW > stickerCanvas.width * 0.8)
            return;
    ...
    document.getElementById('sticker-plus').addEventListener('click', () => {
        if (activeSticker)
            resizeSticker(activeSticker, 1.15);
    });
    document.getElementById('sticker-minus').addEventListener('click', () => {
        if (activeSticker)
            resizeSticker(activeSticker, 0.85);
    });
    // ===== SOURCE TOGGLE =====
            ...
            stickers.forEach(stickerName => {
                ...
                item.addEventListener('click', () => {
                    ...
                    if (isSelected) {
                        ...
                        if (activeSticker && activeSticker.name === stickerName)
                            activeSticker = null;
                        drawStickers();
                    }
                    else {
